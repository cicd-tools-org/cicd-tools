...
_installer_import_support_libraries() {
  ...
  CICD_TOOLS_INSTALLER_TOOLBOX_VERSION="${1}"

  # shellcheck source=/dev/null
  source "$(dirname -- "${BASH_SOURCE[0]}")/../../cicd-tools/boxes/${CICD_TOOLS_INSTALLER_TOOLBOX_VERSION}/libraries/environment.sh"

  # shellcheck source=/dev/null
  source "$(dirname -- "${BASH_SOURCE[0]}")/../../cicd-tools/boxes/${CICD_TOOLS_INSTALLER_TOOLBOX_VERSION}/libraries/logging.sh"

  # shellcheck source=/dev/null
  source "$(dirname -- "${BASH_SOURCE[0]}")/../../cicd-tools/boxes/${CICD_TOOLS_INSTALLER_TOOLBOX_VERSION}/libraries/tools.sh"
}
...
_installer_poetry_init() {
  ...
  #2: boolean to enable or disable package mode

  local CICD_TOOLS_POETRY_PACKAGE_MODE="true"

  if [[ "$2" != "true" ]] && [[ "$2" != "false" ]]; then
    log "ERROR" "The poetry 'package-mode' can be set to either 'true' or 'false'."
    return 127
  fi

  CICD_TOOLS_POETRY_PACKAGE_MODE="false"
...
EOF
  ...
  if ! grep "package-mode" pyproject.toml >> /dev/null; then
    sed -i.bak "s/\[tool.poetry\]/\[tool.poetry\]\\npackage-mode = ${CICD_TOOLS_POETRY_PACKAGE_MODE}/g" pyproject.toml
    rm pyproject.toml.bak
