...
_execute_serial_commands_start() {
  ...
  for SERIAL_COMMAND in "${SERIAL_COMMANDS_ARRAY[@]}"; do
    if [[ -z "${SERIAL_COMMAND_PREFIX}" ]]; then
      ...
      SERIAL_SYSTEM_CALL="${SERIAL_COMMAND_PREFIX} ${SERIAL_COMMAND}"
    fi
...
_execute_serial_commands_usage() {
  ...
  log "ERROR" "USAGE: execute-serial-commands.sh -c [NEWLINE SEPARATED COMMANDS] -e [EXECUTION PATH] -p [COMMAND PREFIX TO PREPEND]"
  exit 127
}
